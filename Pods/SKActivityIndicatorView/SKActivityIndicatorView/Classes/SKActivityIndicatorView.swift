...
public class SKActivityIndicator: NSObject {
    ...
    fileprivate var statusLabelFont        : UIFont                 = UIFont.systemFont(ofSize  : 16, weight  : UIFontWeightSemibold)
    ...
    fileprivate func configureActivityIndicatorSize() {
        ...
        if let statusMessage =  statusLabel?.text, statusMessage.count != 0 {
            let attributes = [NSFontAttributeName: statusLabel?.font] as [String : Any]
            let options: NSStringDrawingOptions = [.usesFontLeading, .truncatesLastVisibleLine, .usesLineFragmentOrigin]
            rectLabel = (statusLabel?.text?.boundingRect(with: CGSize(width: 200, height: 300), options: options, attributes: attributes, context: nil))!
